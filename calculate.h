...
void binary_tree::create_tree(string s)
{
    ...
    float decimal_weight = 0.1f;
    for (int i = 0; i < l; i++)
    {
        if (s[i] >= '0' && s[i] <= '9')
        {
            if(!is_decimal)//integral part
            {
                p->data = (s[i] - '0') + p->data * 10;
            }
            else//decimal part
            {
                p->data += (s[i] - '0') * decimal_weight;
                decimal_weight *= 0.1f;
            }
        }
        else if (s[i] == '.')//deal with '.'
        {
            is_decimal = true;
            decimal_weight = 0.1f;
        }
...
{
    if (root == NULL) return new stack_node();//empty node protect
